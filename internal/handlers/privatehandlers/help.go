...
func (h *helpHandler) handleCommand(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	helpText := "<b>📋 Доступные команды</b>\n\n" +
		...
		"<b>📅 События</b>\n" +
		"• /events - Показать список предстоящих событий\n" +
		"• /topics - Просмотреть темы и вопросы к предстоящим событиям\n" +
		"• /topicAdd - Предложить тему или вопрос к предстоящему событию\n\n" +
	...
	if utils.IsUserAdminOrCreator(b, msg.From.Id, h.config.SuperGroupChatID) {
		adminHelpText := "\n\n<b>🔐 Команды администратора</b>\n" +
			...
			"• /eventSetup - Создать новое событие\n" +
			"• /eventDelete - Удалить событие\n" +
			"• /eventFinish - Отметить событие как завершенное\n" +
			"• /showTopics - Просмотреть темы и вопросы к предстоящим событиям с возможностью удаления\n" +
